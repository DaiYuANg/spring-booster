package org.toolkit4j.framework.spring.boot.starter.restful.configurations.i18n;
...
public class CustomMessageSource extends AbstractMessageSource {
    ...
    private void load(@NotNull File r) {
        ...
        try (FileInputStream fis = new FileInputStream(r)) {
            p.load(fis);
            store.put(key, p.entrySet().stream().map(propEntry -> new AbstractMap.SimpleEntry<>((String) propEntry.getKey(), (String) propEntry.getValue())).collect(Collectors.toSet()));
        }
    }
}
