...
public class CredentialsCentre {
    ...
    private final ExpiringMap<String, String> credentialStore = ExpiringMap.builder()
            .variableExpiration()
            .build();

    private final ExpiringMap<String, RSA> encryptStore = ExpiringMap.builder().variableExpiration().build();
    ...
    public String createCredential(String minioInstance, @NotNull HttpServletRequest request) {
        ...
        val after = StrUtil.str(rsa.encrypt(accessesKey.getBytes(StandardCharsets.UTF_8), KeyType.PublicKey), StandardCharsets.UTF_8);
        credentialStore.put(minioInstance, after,
    ...
    private int uniqueDevice(@NotNull UserAgent userAgent, @NotNull IMinioTemplate template) {
        return Math.abs(userAgent.getOs().getName().hashCode() +
                ...
                template.hashCode() + this.hashCode()
        );
    }
