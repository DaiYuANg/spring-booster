...
public class MinioAutoConfiguration implements WebMvcConfigurer {
    ...
    private @NotNull TemplateEntry createTemplate(Map.@NotNull Entry<String, MinioInstance> config) {
        val client = new MinioClient.Builder()
                ...
                .build();
        val template = config.getValue().isAsync() ? new AsyncMinioTemplate(client) : new MinioTemplate(client);
        return new TemplateEntry(config.getKey(), template);
    }

    private static class TemplateEntry implements Map.Entry<String, IMinioTemplate> {
