...
@Slf4j
@Order(0)
public class AnnotationPermitInstaller implements FeatureInstaller {
    ...
    private @NotNull List<AntPathRequestMatcher> buildAntPathRequestMatcherFromAnnotation() {
        return beanRegistry.getBeanDistinct(RequestMappingHandlerMapping.class).stream().map(RequestMappingHandlerMapping::getHandlerMethods)
    ...
    private Stream<AntPathRequestMatcher> buildAntPathByAnnotation(
        ...
        return Optional.ofNullable(ann)
                ...
                .orElseGet(() -> entry.getValue().hasMethodAnnotation(PermitAll.class)
                        ? Arrays.stream(new AntPathRequestMatcher[]{new AntPathRequestMatcher(path)}).distinct()
