...
public class DefaultDictScannerImpl implements DictScanner {
    ...
    @PostConstruct
    public void init() {
        val allDict = context.getBeansWithAnnotation(Dict.class);
        System.err.println(allDict);
        allDict.values().forEach(d -> {
            val values = Arrays.stream(d.getClass().getDeclaredFields()).filter(field -> field.isAnnotationPresent(DictValue.class));
//            convert to normal pojo and then into cache
//            values.map
        });
    }
