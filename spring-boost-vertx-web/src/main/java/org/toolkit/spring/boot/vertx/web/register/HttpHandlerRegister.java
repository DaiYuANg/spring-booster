...
import org.toolkit.spring.boot.vertx.web.annotation.HttpHandler;
import org.toolkit.spring.boot.vertx.web.base.HttpRouterHandler;
...
public class HttpHandlerRegister {
	...
	@PostConstruct
	public void init() {
		context.getBeansWithAnnotation(HttpHandler.class).values().stream()
	...
	private void attachRoute(@NotNull HttpRouterHandler handler) {
		...
		val route = method.map(m -> router.route(m, ann.path())).orElseGet(() -> router.route(ann.path()));
